...
# test_exit_code=$?
jmeter -n -t test.jmx -JproxyHost="${proxyHost}" -JproxyPort="${proxyPort}" -JproxyScheme="${proxyScheme}" -Jenv="${ENVIRONMENT}" -l "${REPORTFILE}" -j "${LOGFILE}" -o "${JM_REPORTS}" -e -f; test_exit_code=$?
